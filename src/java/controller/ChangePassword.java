...
public class ChangePassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(acc.getPassword().toString().trim().equals(mkCu)){
            if(mkMoi.equals(reMkMoi)){
                
                dao.changePass(acc.getEmail(), mkMoi);
                response.sendRedirect("logout");
            }else{
                 request.setAttribute("mess", "Hai mật khẩu mới không trùng nhau!");
             request.getRequestDispatcher("changePassword.jsp").forward(request, response);
            }
        }else{
             request.setAttribute("mess", "Mật khẩu cũ nhập sai!");
             request.getRequestDispatcher("changePassword.jsp").forward(request, response);
        }
    }
